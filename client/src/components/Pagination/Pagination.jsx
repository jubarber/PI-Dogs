...
export default function Pagination() {
  ...
  return (
    ...
    <div className={estilos.container}>
      <button onClick={handlePrev} className={estilos.btn}>Anterior</button>
      <button className={estilos.btnPage} onClick={handleFirst}>{pageFiltered[0]}</button>
      <div className={estilos.currentPage}>{currentPage}</div>
      <button className={estilos.btnPage} onClick={handleLast}>{pageFiltered.length}</button>
      <button onClick={handleNext} className={estilos.btn}>Siguiente</button>
    </div>
    </div>
  );
}
