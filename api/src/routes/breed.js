  ...
  try {
    ...
    if (name) {
      ...
      const dbBreedsInfo = await Breed.findAll({ include: Temperament }).then(
        (r) => r.filter((e) => e.dataValues.name.toLowerCase().includes(name.toLowerCase()))
      );
      // console.log("SOY DB BREEDS INFO", dbBreedsInfo);
      if (dbBreedsInfo.length !== 0) { 
        const dbPromiseBreeds = dbBreedsInfo.map((e) => { //devuelvo las propiedades que me piden
          return {
            name: e.name,
...
router.get("/:breedId", async (req, res, next) => { //muestra altura peso temperamento, imagen, nombre y anios de vida.
  ...
  try {
    if (breedId.length > 10) {
      ...
      // busco en mi API
      const breedApi = await axios.get(`https://api.thedogapi.com/v1/breeds/${breedId}`);
